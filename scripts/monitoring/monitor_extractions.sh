...
# Shows which files are processing, queued, and completed
...
while true; do
    ...
    STATUS=$(docker compose -f "$REIMS_DIR/docker-compose.yml" exec -T postgres psql -U reims -d reims -c "
        SELECT 
            extraction_status,
            COUNT(*) as count,
            STRING_AGG(SUBSTRING(file_path FROM 'ESP_2025_\d+'), ', ' ORDER BY id) as files
        FROM document_uploads 
        WHERE file_path LIKE '%ESP_2025_%Income_Statement%'
        GROUP BY extraction_status
        ORDER BY extraction_status;
    " 2>&1 | grep -v "rows)" | grep -v "^$" | tail -n +2)
    ...
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║     REIMS2 - Real-Time Extraction Status Monitor                 ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════╝${NC}"
    echo -e "${BLUE}Last Updated: ${TIMESTAMP}${NC}"
    echo ""
    
    # Parse and display counts (pipe-separated values)
    if [ ! -z "$COUNTS" ]; then
    ...
    # Show recent Celery activity
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}🔄 RECENT ACTIVITY:${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    docker compose -f "$REIMS_DIR/docker-compose.yml" logs --tail=5 celery-worker 2>&1 | grep -E "(Starting extraction|Extraction completed|Extraction failed|upload_id)" | tail -3 | sed 's/^/   /' || echo "   No recent activity"
    echo ""
