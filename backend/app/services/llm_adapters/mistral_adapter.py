"""
Mistral API adapter for multi-LLM structured JSON extraction.
"""
import json
import logging
import re
...
class MistralAdapter(BaseLLMAdapter):
    ...
    async def generate_structured_json(
        ...
        if not api_key:
            return LLMCandidateResult(provider=self.provider_name, error="MISTRAL_API_KEY not set")
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                last_error = str(e)
                logger.warning("Mistral attempt %s failed: %s", attempt + 1, e)
        latency_ms = (time.perf_counter() - start) * 1000
        return LLMCandidateResult(
            provider=self.provider_name,
            model=model,
