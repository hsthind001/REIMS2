...
class ExtractionRun(Base):
    ...
    Uses JSON (not JSONB) so the model works on both SQLite (tests) and PostgreSQL.
    """
    ...
    master_json = Column(JSON, nullable=True)

    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), onupdate=func.now(), nullable=True)

    # Many-to-one to DocumentUpload. No back_populates to avoid "both MANYTOONE" (DocumentUpload has viewonly reverse).
    document_upload = relationship(
        "DocumentUpload",
        foreign_keys=[document_upload_id],
    )
